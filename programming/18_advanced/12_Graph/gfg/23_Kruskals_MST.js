...
class DisjointSet {
    constructor(size) {
        ...
        this.rank = new Array(size).fill(0); // Rank = HEIGHT(Tree Depth) (standard textbook DSU)
    ...
    union(x, y) { // ✅ Union by Rank
        ...
        if (this.rank[rootX] < this.rank[rootY]) {
            ...
            this.parent[rootY] = rootX;
            this.rank[rootX]++; // rank changes only when ranks are equal (i.e. increase rank only when two trees of equal height merge.)
        }
