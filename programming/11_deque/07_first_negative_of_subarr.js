...
function firstNegativeInWindow(arr, K) {
    const result = [];
    const deq = new Deque(K) // Stores indices of negative numbers
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        // The front of the deque is the first negative in the window
        if (i >= K - 1) {
            result.push(!deq.isEmpty() ? arr[deq.getFront()] : 0);
        }
    }
    return result;
}
...
let arr1 = [-8, 2, 3, -6, 10]
let K1 = 2
console.log(firstNegativeInWindow(arr1, K1).join(" "))
// Output: -8 0 -6 -6

let arr2 = [1, 2, 3, 4, -1, 8, 10, -7, 15]
let K2 = 3
console.log(firstNegativeInWindow(arr2, K2).join(" "))
// Output: 0 0 -1 -1 -1 -7 -7